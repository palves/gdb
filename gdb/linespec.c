...
static const char *
skip_quote_char (enum language language, const char *string, char quote_char)
{
  const char *p, *last;

  p = last = find_toplevel_char (language, string, quote_char);
  while (p && *p != '\0' && *p != ':')
    {
      p = find_toplevel_char (language, p, quote_char);
      if (p != NULL)
	last = p++;
    }
...
linespec_lexer_lex_string (linespec_parser *parser)
{
  ...
  if (strchr (linespec_quote_characters, *PARSER_STREAM (parser)))
	  ...
	  && quote_char == '\"')
	{
      ...
      end = skip_quote_char (PARSER_STATE (parser)->language->la_language,
			     PARSER_STREAM (parser), quote_char);
				   ...
				   int include_inline)
{
  ...
  ALL_PSPACES (pspace)
  {
      ...
      ALL_OBJFILE_COMPUNITS (objfile, cu)
	{
	  ...
	  iterate_over_file_blocks (symtab, name, matcher_data.symbol_name_cmp,
	  ...
	  if (include_inline)
		   ...
		   i++)
		{
		  ...
		  state->language->la_iterate_over_symbols
		    (block, name, name_language, matcher_data.symbol_name_cmp,
			  ...
			  symbol_found_callback_ftype *callback, void *data)
{
  ...
  for (block = BLOCKVECTOR_BLOCK (SYMTAB_BLOCKVECTOR (symtab), STATIC_BLOCK);
    ...
    LA_ITERATE_OVER_SYMBOLS (block, name, symbol_compare,
			     domain, callback, data);
}
...
const char *
find_toplevel_char (enum language language, const char *s, char c)
{
...
static const char *
find_toplevel_string (enum language language,
		      const char *haystack, const char *needle)
{
  ...
  do
    {
      s = find_toplevel_char (language, s, *needle);
...
linespec_complete_function (completion_tracker &tracker,
			    ...
			    const char *text, const char *source_filename)
{
  complete_symbol_mode mode = complete_symbol_mode::LINESPEC;
  compare_symbol_name_ftype *compare_name
    = current_language->la_get_compare_symbol_name (text, strlen (text));
  const char *global_ns_prefix
    = global_namespace_search_name_prefix (language);

  if (global_ns_prefix != NULL
      && startswith (text, global_ns_prefix))
    {
      size_t len = strlen (global_ns_prefix);

      tracker.advance_custom_word_point_by (len);
      text += len;
    }

  if (source_filename != NULL)
    {
      collect_file_symbol_completion_matches (tracker, mode, compare_name,
					      text, text, source_filename);
    }
  else
    {
      collect_symbol_completion_matches (tracker, mode, compare_name,
					 text, text);
			     ...
			     const char *source_filename)
{
  if (component == linespec_complete_what::KEYWORD)
  ...
  else if (component == linespec_complete_what::FUNCTION)
    {
      ...
      linespec_complete_function (tracker, parser->state.language->la_language,
  ...
  if (parser.complete_what == linespec_complete_what::LABEL)
    {
      ...
      find_linespec_symbols (PARSER_STATE (&parser),
			     PARSER_RESULT (&parser)->file_symtabs,
			     func_name, &function_symbols, &minimal_symbols);
		       ...
		       VEC (bound_minimal_symbol_d) **minsyms)
{
  ...
  /* In e.g. C++, if the user wanted an exact match, preserve the
     initial "::" in the lookup name.  */
  std::string gl_ns_lookup_storage;
  enum language language = state->language->la_language;
  const char *global_ns_prefix = global_namespace_search_name_prefix (language);
  if (global_ns_prefix != NULL
      && startswith (name, global_ns_prefix))
    {
      gl_ns_lookup_storage = std::string (global_ns_prefix) + lookup_name;
      lookup_name = gl_ns_lookup_storage.c_str ();
    }
  ...
  if (VEC_empty (symbolp, *symbols)
      && VEC_empty (bound_minimal_symbol_d, *minsyms))
    {
	 ...
	 name into namespaces${SCOPE_OPERATOR}class_name and method_name.  */
      scope_op = "::";
      p = find_toplevel_string (state->language->la_language,
				lookup_name, scope_op);
...
static int
strncmp_iw_adapter (const char *symbol_search_name,
		    const char *string2)
{
  return strncmp_iw (symbol_search_name, string2, strlen (string2));
}
  ...
  if (completion_mode)
    {
      ...
      ALL_BLOCK_SYMBOLS_WITH_NAME (block, name, SYMBOL_LANGUAGE (fn_sym),
				   strncmp_iw_adapter, iter, sym)
	{
			 ...
			 struct symtab *symtab)
{
  ...
  symbol_name_cmp_ftype *symbol_name_cmp
    = (info->state->language->la_get_symbol_name_cmp != NULL
       ? info->state->language->la_get_symbol_name_cmp (name)
       : strcmp_iw);
