...
/* NULL-terminate so this can be used as enum completer.  */
...
static void iterate_over_file_blocks
  (struct symtab *symtab, const char *name,
...
const char *
psymbol_unqualified_name (const char *name)
{
  /* Might be a C/C++/Rust/Fortran name.  */
  unsigned int len = cp_entire_prefix_len (name);
  if (len != 0)
    return name + len;

  size_t namelen = strlen (name);
  const char *p = name + namelen;

  for (; p >= name; p--)
    {
      /* Might be an Ada, D or Go name.  */
      if (p[0] == '.' && p[1] != '\0')
	return p + 1;

      /* Might be an Ada encoded name.  */
      if (p[0] == '_' && p[1] == '_' && p[2] != '_' && p[2] != '\0')
	return p + 2;
    }
...
/* A helper function object for iterate_over_all_matching_symtabs that
   is passed as a callback to the expand_symtabs_matching method.  */

class symbol_matcher
{
public:
  symbol_matcher (const char *name,
		  symbol_name_match_type name_match_type)
    : m_lookup_name (name),
      m_lookup_unqualified_name (psymbol_unqualified_name (name)),
      m_name_match_type (name_match_type)
  {}

  /* Callable as a expand_symtabs_symbol_matcher_ftype callback.  */
  bool operator () (const char *name);

private:
  /* The lookup name against which symbol name should be compared.  */
  const char *m_lookup_name;

  /* The unqualified name part of lookup_name.  */
  const char *m_lookup_unqualified_name;

  /* The routine to be used for comparison XXX .  */
  symbol_name_match_type m_name_match_type;
};

bool
symbol_matcher::operator () (const char *name)
{
  ...
  if (strcasecmp (m_lookup_unqualified_name, unqual_name) == 0)
    {
      for (int i = 0; i < nr_languages; i++)
	{
	  const language_defn *lang = language_def ((enum language) i);

	  if (lang->la_get_symbol_name_cmp != NULL)
	    {
	      symbol_name_cmp_ftype *cmp
		= lang->la_get_symbol_name_cmp (m_name_match_type,
						m_lookup_name);
	      if (cmp (name, m_lookup_name) == 0)
		return true; /* Expand this symbol's symbol table.  */
	    }
	}

      if (strcmp_iw (name, m_lookup_name) == 0)
	return true; /* Expand this symbol's symbol table.  */
    }
...
iterate_over_all_matching_symtabs
  (struct linespec_state *state, const char *lookup_name,
   symbol_name_match_type name_match_type,
   ...
   gdb::function_view<symbol_found_callback_ftype> callback)
{
  ...
  symbol_matcher matcher (lookup_name, name_match_type);
  ...
  ALL_PSPACES (pspace)
  {
      ...
      ALL_OBJFILE_COMPUNITS (objfile, cu)
	{
	  ...
	  iterate_over_file_blocks (symtab, lookup_name, name_match_type,
				    domain, callback);
	  ...
	  if (include_inline)
		   ...
		   i++)
		{
		  ...
		  state->language->la_iterate_over_symbols
		    (block, lookup_name, name_match_type, domain,
		     [&] (symbol *sym)
		     {
...
iterate_over_file_blocks
  (struct symtab *symtab, const char *name,
   symbol_name_match_type compare_name, domain_enum domain,
   gdb::function_view<symbol_found_callback_ftype> callback)
{
  struct block *block;
  ...
  for (block = BLOCKVECTOR_BLOCK (SYMTAB_BLOCKVECTOR (symtab), STATIC_BLOCK);
    ...
    LA_ITERATE_OVER_SYMBOLS (block, name, compare_name, domain, callback);
}
...
linespec_parse_basic (linespec_parser *parser)
{
  ...
  if (parser->completion_tracker != NULL)
	  ...
	  && strcmp (PARSER_STREAM (parser), ":") == 0)
	{
	  ...
	  if (!tmp_tracker.m_entries_vec.empty ())
	    {
	      PARSER_STREAM (parser)++;
	      LS_TOKEN_STOKEN (token).length++;
  ...
  if (token.type == LSTOKEN_EOI)
	...
	{
	  /* User specified an offset.  Record the line offset and get
	     the next token.  */
	  set_completion_after_number (parser, linespec_complete_what::KEYWORD);
  ...
  if (function_name != NULL)
    {
      find_linespec_symbols (self, result->file_symtabs,
			     function_name, fname_match_type, &symbols,
			     &minimal_symbols);
			     ...
			     const char *source_filename)
{
  if (component == linespec_complete_what::KEYWORD)
  ...
  else if (component == linespec_complete_what::FUNCTION)
    {
      ...
      linespec_complete_function (tracker, text,
				  symbol_name_match_type::WILD, source_filename);
      if (source_filename == NULL)
	fn_list = complete_source_filenames (text);
	 ...
	 the filename.  */
      if (fn_list.size () == 1 && tracker.m_entries_vec.empty ())
	{
  ...
  for (int ix = 0;
       VEC_iterate (symbolp, labels, ix, label); ++ix)
    {
      completion_list_add_name (tracker,
				...
				/* Should we have a simpler version for this (strcmp_iw) ?  */
				symbol_name_match_type::FULL,
				language_c, /* XXXX */
				SYMBOL_SEARCH_NAME (label),
				label_name,
				strlen (label_name),
  ...
  if (parser.complete_what == linespec_complete_what::LABEL)
	...
	= find_label_symbols (PARSER_STATE (&parser),
      ...
      for (int ix = 0;
	   VEC_iterate (symbolp, labels, ix, label); ++ix)
	{
	  completion_list_add_name (tracker, complete_symbol_mode::LINESPEC,
				    symbol_name_match_type::FULL,
				    /// XXX
				    language_c,
				    SYMBOL_SEARCH_NAME (label),
				    parser.completion_word,
				    strlen (parser.completion_word),
				    text, word);
	}
      VEC_free (symbolp, labels);
    }
      ...
      if (tracker.quote_char ())
	{
      ...
      else if (tracker.m_entries_vec.empty ())
	{
	  size_t key_start;
      ...
      else if (completes_to_completion_word (tracker, word))
	{
...
decode_objc (struct linespec_state *self, linespec_p ls, const char *arg)
{
  ...
  cleanup = make_cleanup (VEC_cleanup (const_char_ptr),
			  &symbol_names);
		   ...
		   const char *class_name)
{
  ...
  /// should "component name, or some such?
  ...
  for (ix = 0; VEC_iterate (symtab_ptr, file_symtabs, ix, elt); ++ix)
    {
      if (elt == NULL)
	{
	  iterate_over_all_matching_symtabs (state, class_name, match_type,
					     STRUCT_DOMAIN, NULL, false, collector);
	  iterate_over_all_matching_symtabs (state, class_name, match_type,
      ...
      else
	{
	  ...
	  iterate_over_file_blocks (elt, class_name, match_type,
			 ...
			 struct symtab *symtab)
{
  ...
  if (symtab == NULL)
	...
	ALL_OBJFILES (objfile)
	{
	  ...
	  iterate_over_minimal_symbols (objfile, name, name_match_type,
					add_minsym, &local);
	}
      }
    }
      ...
      if (search_pspace == NULL || SYMTAB_PSPACE (symtab) == search_pspace)
	{
	  ...
	  iterate_over_minimal_symbols (local.objfile, name, name_match_type,
					add_minsym, &local);
	}
    }
  ...
  for (ix = 0; VEC_iterate (symtab_ptr, info->file_symtabs, ix, elt); ++ix)
    {
      if (elt == NULL)
	{
	  iterate_over_all_matching_symtabs (info->state, name,
					     name_match_type, VAR_DOMAIN,
					     pspace, true, [&] (symbol *sym)
	    { return info->add_symbol (sym); });
	  search_minsyms_for_name (info, name, name_match_type, pspace, NULL);
	}
      else if (pspace == NULL || pspace == SYMTAB_PSPACE (elt))
	{
	  ...
	  gdb_assert (!SYMTAB_PSPACE (elt)->executing_startup);
	  set_current_program_space (SYMTAB_PSPACE (elt));
	  iterate_over_file_blocks (elt, name, name_match_type, VAR_DOMAIN,
				    [&] (symbol *sym)
	    { return info->add_symbol (sym); });
	  ...
	  if (prev_len == VEC_length (symbolp, info->result.symbols)
	    ...
	    search_minsyms_for_name (info, name, name_match_type, pspace, elt);
	}
    }
}
