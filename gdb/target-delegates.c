...
static int
delegate_is_non_stop_p (struct target_ops *self)
{
  self = self->beneath;
  return self->to_is_non_stop_p (self);
}

static int
tdefault_is_non_stop_p (struct target_ops *self)
{
  return 0;
}

static int
debug_is_non_stop_p (struct target_ops *self)
{
  int result;
  fprintf_unfiltered (gdb_stdlog, "-> %s->to_is_non_stop_p (...)\n", debug_target.to_shortname);
  result = debug_target.to_is_non_stop_p (&debug_target);
  fprintf_unfiltered (gdb_stdlog, "<- %s->to_is_non_stop_p (", debug_target.to_shortname);
  target_debug_print_struct_target_ops_p (&debug_target);
  fputs_unfiltered (") = ", gdb_stdlog);
  target_debug_print_int (result);
  fputs_unfiltered ("\n", gdb_stdlog);
...
install_delegators (struct target_ops *ops)
{
  ...
  if (ops->to_is_non_stop_p == NULL)
    ops->to_is_non_stop_p = delegate_is_non_stop_p;
...
install_dummy_methods (struct target_ops *ops)
{
  ...
  ops->to_is_non_stop_p = tdefault_is_non_stop_p;
...
init_debug_target (struct target_ops *ops)
{
  ...
  ops->to_is_non_stop_p = debug_is_non_stop_p;
