...
set_current_context (void)
{
  ...
  if (thr != NULL)
    current_execution_context.thread_gnum = thr->global_num;
  else
	 ...
	 execution command further.  */
	  ...
	  if (itset_width_contains_thread (run_free_itset, default_width, t)
		  ...
		  || t->pending_follow.kind == TARGET_WAITKIND_VFORKED)
		{
	      ...
	      else if (thread_needs_step_over (tp))
		{
...
{
  struct step_1_args *args = (struct step_1_args *) data;
...
ensure_runnable (struct thread_info *thr)
{
  if (thr->state == THREAD_EXITED)
    error (_("Thread %s (%s) has exited."),
	   print_thread_id (thr), target_pid_to_str (thr->ptid));
  else if (thr->state == THREAD_RUNNING)
    error (_("Thread %s (%s) is running."),
	   print_thread_id (thr), target_pid_to_str (thr->ptid));
}
...
{
  struct jump_aec_callback_data *arg = (struct jump_aec_callback_data *) data;
  struct jump_map_entry *jme;
  int ix;
  struct gdbarch *gdbarch = get_current_arch ();
  int from_tty = arg->from_tty;
  struct jump_cmd_data *cmd_data = (struct jump_cmd_data *) thread->cmd_data;
  CORE_ADDR addr = cmd_data->addr;

  if (from_tty)
    printf_filtered (_("Continuing %s (%s) at %s.\n"),
...
{
  struct signal_aec_callback_data *d
    = (struct signal_aec_callback_data *) data;
...
{
  struct finish_aec_callback_data *d
    = (struct finish_aec_callback_data *) data;
  int from_tty = d->from_tty;
  struct finish_cmd_data *cmd_data = (struct finish_cmd_data *) tp->cmd_data;
  int ix;
