...
/* Compute the demangled form of NAME as used by the various symbol
   ...
   directly, or a pointer to a buffer owned by by the STORAGE object.
  ...
  if (lang == language_cplus)
    {
      ...
      if (demangled_name != NULL)
	{
	  storage.m_malloc.reset (demangled_name);
      ...
      else
	{
	  ...
	  if (!canon.empty ())
	    {
	      std::swap (storage.m_string, canon);
	      modified_name = storage.m_string.c_str ();
	    }
	}
    }
