...
remote_target::discard_pending_stop_replies (struct inferior *inf)
{
  ...
  if (reply != NULL && reply->ptid.pid () == inf->pid)
    {
      /* Leave the notification pending, since we must acknowledge it
	 with vStopped.  But clear it, so that later on acknowledging
	 discard it.  */
      reply->ws.kind = TARGET_WAITKIND_IGNORE;

      if (remote_debug)
	fprintf_unfiltered (gdb_stdlog,
			    "XXX discarded in-flight notification!\n");
    }
