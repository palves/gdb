...
public:
  ...
  /* A map of ptid_t to thread_info*, for average O(1) ptid_t lookup.
     Not that exited threads do not appear in the map.  */
  std::unordered_map<ptid_t, thread_info *, hash_ptid_t> ptid_thread_map;
...
/* Iterator over inferior_list.  Note that dereferencing this iterator
   returns an inferior reference is as normal with standard library
   iterators, not a pointer.  Most code in GDB works with pointers for
   legacy reasons, so will use all_inferiors() instead, which iterates
   over inferiors using pointers.  See inferior-iter.h.  */
