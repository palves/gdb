	  ...
	  if (!lanes.empty ())
	    {
		 ...
		 arguments at least for the first one.  The current lane will
	      ...
	      tp->set_current_simd_lane (lanes[0]);
	    }
...

  {
      ...
      for (thread_info *tp : inf->threads ())
	{
	  ...
	  if (active_lanes_mask != 0)
		...
		print_thread_row (uiout, tp, true, false, active_lanes_to_display,
				  show_global_ids);
	  ...
	  else
	    {
		 ...
		 and do not show any SIMD lanes.  */
	      print_thread_row (uiout, tp, false, current, 0x0, show_global_ids);
	    }
	}
...
(thread_info *tp):
  m_tp {tp},
  m_simd_lane_num {tp->current_simd_lane ()}
{
  m_tp->incref ();
}
...
scoped_restore_current_simd_lane::~scoped_restore_current_simd_lane ()
{
  /* The M_TP thread object is not deleted, because we increased
     its refcount.  */
      ...
      else
	{
	  ...
	  if (tp->has_simd_lanes ())
	      ...
	      if (is_active)
		{
		  uiout->text (" lane ");
		  int lane = tp->current_simd_lane ();
		  uiout->text ((std::to_string (lane)).c_str ());
		  uiout->text (")]");
		}
	      else
		uiout->text (") inactive]");
