...
/* Register PROP as the a an expression for BLOCK in the HTAB map,
   which are part of OBJFILE.  Must not be called more than once for
   each BLOCK.  */
...
/* Register ACTIVE_LANE as the active lane expression for FUNCTION,
   which is part of OBJFILE.  Must not be called more than once for
   each BLOCK.  */

void
objfile_register_active_lane (struct objfile *objfile,
			      const block *function,
			      const dynamic_prop *active_lane)
{
  objfile_register_block_prop (objfile,
			       &objfile->active_lanes,
			       function,
			       active_lane);
}
...
const dynamic_prop *
objfile_lookup_active_lane (struct objfile *objfile,
			    const block *function)
{
  return objfile_lookup_block_prop (objfile,
				    &objfile->active_lanes,
