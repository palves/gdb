...
static ULONGEST
get_eflags (struct regcache *regcache)
{
  int regnum = (register_size (regcache->arch (), 0) == 8
		? (int) AMD64_EFLAGS_REGNUM
		: (int) I386_EFLAGS_REGNUM);
  return regcache_raw_get_unsigned (regcache, regnum);
}

static CORE_ADDR
get_sp (struct regcache *regcache)
{
  int regnum = (register_size (regcache->arch (), 0) == 8
	...
	if (mod == 3 /* 11b */) /* register direct, register in r/m.  */
	  {
	    ...
	    *dest = regcache_raw_get_unsigned (regcache, regnum);
	  }
	...
	else
	  {
		...
		if (index == 4)
		...
		else
		  {
		    ...
		    index_val = regcache_raw_get_unsigned (regcache, index_reg);
		  }
		...
		if (base == 5 && mod == 0)
		...
		else
		  {
		    ...
		    base_val = regcache_raw_get_unsigned (regcache, base_reg);
		  }
		...
		*dest = read_memory_unsigned_integer (eaddress,
		...
		regnum = arch_reg_to_regnum (gdbarch, rm);
		addr = regcache_raw_get_unsigned (regcache, regnum);
		addr += disp;
...
static CORE_ADDR
i386_get_next_pc_1 (struct regcache *regcache, CORE_ADDR pc)
{
  struct gdbarch *gdbarch = regcache->arch ();
  enum bfd_endian byte_order = gdbarch_byte_order (gdbarch);
  CORE_ADDR next_pc;
  ...
  gdb_byte *buf = (gdb_byte *) alloca (len * 2);
  gdb_byte *insn = buf;
  CORE_ADDR org_pc = pc;
  ...
  if (i386_syscall_p (insn, &len))
    {
	  ...
	  && tdep->syscall_next_pc (regcache, &return_addr))
	return return_addr;
...
i386_get_next_pc (struct regcache *regcache, CORE_ADDR pc)
{
  CORE_ADDR next_pc = i386_get_next_pc_1 (regcache, pc);
...
{
  return i386_get_next_pc (regcache, regcache_read_pc (regcache));
}
