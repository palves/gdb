...
{
  /* Avoid setting the breakpoint at an instruction that can avoid a
     fixup phase, like a branch/jump.  In such a case, even GDB
     manages to detach the inferior with an incomplete displaced step,
     GDB inferior may still not crash.  A breakpoint at a line that
     increments a variable is good bet that we end up setting a
     breakpoint at an instruction that will require moving the PC from
     the scratch pad to the instruction after the breakpoint.  */
