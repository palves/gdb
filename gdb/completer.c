...
bool
completes_to_completion_word (completion_tracker &tracker, const char *word)
{
  if (tracker.m_lowest_common_denominator_unique)
    {
      const char *lcd = tracker.m_lowest_common_denominator;
...
/* Complete on symbols, file symbols and files.
			...
			const char *text, const char *word)
{
  ...
  if (colon)
    {
      collect_file_symbol_completion_matches (tracker,
					      ...
					      compare_symbol_name_for_expressions,
					      symbol_start, word,
					      file_to_match);
      xfree (file_to_match);
    }
  ...
  if (tracker.m_entries_vec.empty ())
    {
...
/* Convert an index into the EXPLICIT_OPTIONS array above to an
   explicit_location_match_type.  */

static bool
explicit_location_match_type_from_explicit_option (explicit_location_match_type *result,
						   size_t option_index)
{
  switch (option_index)
    {
				   ...
				   const struct language_defn *language)
{
  ...
  switch (what)
	...
	linespec_complete_function (tracker, language->la_language,
				    function, explicit_loc->source_filename);
      }
      break;
	...
	linespec_complete_label (tracker, language,
				 ...
				 explicit_loc->function_name, label);
      }
      break;
  ...
  if (completes_to_completion_word (tracker, word))
    {
      ...
      else if (wordlen > 0 && keyword[-1] == ' ')
	{
	  /* Assume that we're maybe past the explicit location
	     ...
	     the user wants to create pending breakpoint.  Offer the
			    ...
			    const char *quoted_arg_end)
{
  ...
  if (keyword == -1)
      ...
      if (quoted_arg_start != NULL && quoted_arg_end != NULL)
	{
      ...
      if (!explicit_location_match_type_from_explicit_option (&what, keyword))
	{
		    ...
		    const char *text, const char *word_ignored)
{
  ...
  int saved_word_point = tracker.m_custom_word_point;

  const char *copy = text;

  explicit_completion_info completion_info;
  location = string_to_explicit_location (&copy, current_language,
					  &completion_info);
  ...
  if (location != NULL)
      ...
      else
	{
	  ...
	  complete_explicit_location (tracker, location, text,
				      current_language,
  ...
  */
  if ((text[0] == '-' || text[0] == '\0')
      && (tracker.m_entries_vec.empty ()
	  || tracker.m_custom_word_point == saved_word_point))
    {
      tracker.m_custom_word_point = saved_word_point;
      text = option_text;
...
/* Internal function used to handle completions.
   ...
   XXXXX Preliminary phase, called by gdb_completion_word_break_characters
   function, is used to determine the correct set of chars that are
   word delimiters depending on the current command in line_buffer.
   In this phase, TEXT is NULL because that will be computed based on
   the result of this phase.  No completion list should be generated;
   the return value should be NULL.  This is checked by an assertion
   in that function.
					  ...
					  const char *match_for_lcd)
{
  ...
  if (*slot == HTAB_EMPTY_ENTRY)
	...
	match_for_lcd = name.get ();
      recompute_lowest_common_denominator (match_for_lcd);
...
completion_tracker::add_completions (completion_list &&list)
{
  ...
  /* XXXXXXXXXXX Do a final test for too many completions.  Individual completers may
		  ...
		  const char *text, const char *word)
{
  ...
  for (signum = GDB_SIGNAL_FIRST; signum != GDB_SIGNAL_LAST; ++signum)
      ...
      if (strncasecmp (signame, word, len) == 0)
	{
	  ...
	  tracker.add_completion (std::move (copy), signame);
	}
    }
}
	  ...
	  if (*name != '\0' && strncmp (word, name, len) == 0)
	    {
	      ...
	      tracker.add_completion (std::move (copy), name);
	    }
	}
    }
...
{
  completion_list &list = tracker.m_entries_vec;	/* The completions.  */
  ...
  match_list[0]
    = expand_preserving_ws (text, end - start,
			    tracker.m_lowest_common_denominator);
...
/* Generate the completions for the completer in one go.
...
/* Function installed as "rl_attempted_completion_function" readline
   hook.  This is called by readline to build the match list array,
   and determining the lowest common denominator.  The real matches
...
gdb_rl_attempted_completion_function_throw (const char *text, int start, int end)
{
  ...
  completion_result result
    = build_completion_result (tracker, text, start, end);
