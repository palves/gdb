...
tui_io (void)
{
  ...
  if (ts->io_data == NULL)
    ts->io_data = XCNEW (struct tui_io_data);
...
tui_initialize_io (void)
{
  ...
  tui_io ()->tui_stdout = tui_fileopen (current_ui->outstream);
  tui_io ()->tui_stderr = tui_fileopen (current_ui->errstream);
...
#else
  tui_rl_outstream = current_ui->outstream);
#endif
}
