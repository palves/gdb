...
typedef struct notif_client
{
  ...
  /* Parse BUF to get the expected event and update EVENT.  This
     ...
     expected event.  */
  void (*parse) (remote_target *target,
  ...
  void (*ack) (remote_target *target,
	       struct notif_client *self, char *buf,
	       struct notif_event *event);

  /* Check this notification client can get pending events in
     'remote_notif_process'.  */
  int (*can_get_pending_events) (remote_target *target,
...
void remote_notif_ack (remote_target *target, struct notif_client *nc, char *buf);
struct notif_event *remote_notif_parse (remote_target *target,
...
struct remote_notif_state *remote_notif_state_allocate (remote_target *target);
