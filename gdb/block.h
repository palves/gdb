...
#define ALL_BLOCK_SYMBOLS_WITH_NAME(block, name, name_compare, iter, sym) \
  for ((sym) = block_iter_match_first ((block), (name), (name_compare), \
				       &(iter));			\
       (sym) != NULL;							\
       (sym) = block_iter_match_next ((name), (name_compare), &(iter)))
