...
/* Handle one event after stopping threads (attempting to stability
   one or more inferiors).  If the eventing thread reports back any
   interesting event, we leave it pending.  If the eventing thread was
   in the middle of a displaced step, we cancel/finish it.  */
...
bool
switch_back_to_stepped_thread_tail (process_stratum_target *resume_target,
				    ptid_t resume_ptid);
...
switch_back_to_stepped_thread (struct execution_control_state *ecs)
{
  if (!target_is_non_stop_p ())
	...
	ecs->event_thread->suspend.stop_signal = GDB_SIGNAL_0;
      ...
      if (switch_back_to_stepped_thread_tail (ecs->target, ecs->ptid))
	{
	  prepare_to_wait (ecs);
	  return true;
	}
...
keep_going_XXX ()
{
  ...
  process_stratum_target *resume_target = current_inferior ()->process_target ();

  for (thread_info *thr : all_threads (resume_target))
    {
  ...
  if (switch_back_to_stepped_thread_tail (nullptr, minus_one_ptid))
    return;
