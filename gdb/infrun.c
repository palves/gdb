...
do_target_resume (ptid_t resume_ptid, int step, enum gdb_signal sig)
{
  ...
  /* Advise target which signals may be handled silently.
     ...
     (in any thread), we need to receive all signals to avoid
     ...
     - a backtrace while stopped in the signal handler would show the
       ...
       the real main line code.

     - when the thread is later resumed, the signal handler returns to
       the scratch pad, which would no longer be valid.  */
