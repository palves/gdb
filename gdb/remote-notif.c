...
void
remote_notif_ack (remote_target *target,
		  struct notif_client *nc, char *buf)
{
  ...
  nc->parse (target, nc, buf, event);
  nc->ack (target, nc, buf, event);
...
struct notif_event *
remote_notif_parse (remote_target *target,
		    struct notif_client *nc, char *buf)
{
  struct notif_event *event = nc->alloc_event ();
  struct cleanup *old_chain
  ...
  while (!QUEUE_is_empty (notif_client_p, state->notif_queue))
    {
      ...
      if (nc->can_get_pending_events (state->target, nc))
	remote_notif_get_pending_events (state->target, nc);
    }
}
...
handle_notification (struct remote_notif_state *state, char *buf)
{
  ...
  if (state->pending_event[nc->id] != NULL)
	...
	= remote_notif_parse (state->target, nc, buf + strlen (nc->name) + 1);
...
remote_notif_state_allocate (remote_target *target)
{
  struct remote_notif_state *notif_state = XCNEW (struct remote_notif_state);

  notif_state->target = target;
