...
bpstat_check_breakpoint_conditions (bpstat bs, ptid_t ptid)
{
  ...
  /* Likewise if the breakpoint has a trigger set, and it doesn't
     match.  */
  else if (b->trigger_set != NULL)
    {
      struct thread_info *thread = find_thread_ptid (ptid);

      if (!bpstat_check_trigger_set (b, thread))
	{
	  bs->stop = 0;
	  return;
	}
...
/* Tell us what we should suspend.  Returns a union of the suspend
   sets of all breakpoints that caused the stop.  */

struct itset *
bpstat_suspend_set (bpstat bs_head)
{
  bpstat bs;
  struct itset *suspend_set = itset_create_empty ();
  ...
  for (bs = bs_head; bs != NULL; bs = bs->next)
	...
	continue;
      ...
      if (bs->breakpoint_at->suspend_set != NULL)
	itset_add_set (suspend_set, bs->breakpoint_at->suspend_set);
    }
		 ...
		 const struct breakpoint_ops *ops)
{
  ...
  b->trigger_set = itset_reference (trigger_set);
  b->suspend_set = itset_reference (suspend_set);
...
struct cleanup *
parse_breakpoint_args_const (const char **args,
			     struct itset **trigger_set,
			     struct itset **suspend_set)
{
  struct cleanup *old_chain;

  old_chain = default_breakpoint_itsets (trigger_set, suspend_set);
  ...
  if (*args != NULL)
      ...
      while (*arg != '\0')
	{
	  ...
	  p = skip_to_space_const (arg);

	  if (strncmp (arg, "-suspend", p - arg) == 0)
	    {
	      p = skip_spaces_const (p);
...
parse_breakpoint_args (char **args,
		       ...
		       struct itset **suspend_set)
{
  const char *args_const;
  struct cleanup *old_chain;

  args_const = *args;
  old_chain = parse_breakpoint_args_const (&args_const,
					   trigger_set, suspend_set);
  *args = (char *) args_const;
...
static void
watch_command_2 (struct itset *trigger_set, struct itset *suspend_set,
		 const char *arg, int accessflag, int from_tty,
		 ...
		 int just_location, int internal)
{
  ...
  old_chain = parse_breakpoint_args_const (&arg, &trigger_set, &suspend_set);

  watch_command_2 (trigger_set, suspend_set, arg, accessflag, from_tty,
		   just_location, internal);
...
watch_maybe_just_location (char *arg, int accessflag, int from_tty)
{
  ...
  watch_command_2 (trigger_set, suspend_set,
		   arg, accessflag, from_tty, just_location, 0);
...
until_break_aec_callback (struct thread_info *thread, void *data)
{
  ...
  struct until_break_cmd_data *cmd_data
    = (struct until_break_cmd_data *) thread->cmd_data;
  int ix;
  struct until_break_fsm *sm;
  struct cleanup *old_chain;
  ...
  location_breakpoint = set_momentary_breakpoint (get_frame_arch (frame),
						  cmd_data->sal,
  ...
  thread->thread_fsm = &sm->thread_fsm;
		      ...
		      struct cmd_list_element *command)
{
  ...
  switch (fork_kind)
    ...
    case catch_fork_permanent:
      create_fork_vfork_event_catchpoint (trigger_set, suspend_set,
			     ...
			     int internal, unsigned flags)
{
  create_breakpoints_sal_default (gdbarch, canonical,
				  ...
				  trigger_set, suspend_set, thread, task,
				   ...
				   int internal, unsigned flags)
{
  create_breakpoints_sal_default (gdbarch, canonical,
				  cond_string, extra_string,
				  type_wanted,
				  disposition, NULL, NULL, thread, task,
				  ignore_count, ops, from_tty,
				  enabled, internal, flags);
}
...
trace_command (char *arg, int from_tty)
{
  ...
  back_to = parse_breakpoint_args (&arg, &trigger_set, &suspend_set);
  location = string_to_event_location (&arg, current_language);
  make_cleanup_delete_event_location (location);
  if (location != NULL
      && event_location_type (location) == PROBE_LOCATION)
...
create_tracepoint_from_upload (struct uploaded_tp *utp)
{
  ...
  cleanup = default_breakpoint_itsets (&trigger_set, &suspend_set);
