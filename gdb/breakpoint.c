  ...
  if (!part_of_multiple && b->trigger_set != NULL)
    {
      ...
      // here simply "trigger only in ..." "suspend all in ..."
...
void
parse_breakpoint_args (const char **args,
		       struct itset **trigger_set,
		       struct itset **stop_set);
		   ...
		   unsigned flags)
{
  ...
  // XXXXX fixme cast
  parse_breakpoint_args ((const char **) &arg, &trigger_set, &stop_set);
			     ...
			     int internal, unsigned flags)
{
  create_breakpoints_sal_default (gdbarch, canonical,
				  ...
				  trigger_set, stop_set, thread, task,
