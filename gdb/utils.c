...
cp_skip_operator_token (const char *token, const char *end)
{
  ...
  while (p != end && !isspace (*p) && *p != '(')
    {
      if (valid_symbol_name_char (*p))
	{
	  while (p != end && valid_symbol_name_char (*p))
	    p++;
	  return p;
	}
      else
...
{
  return ((string == start
	   || !valid_symbol_name_char (string[-1]))
	  && strncmp (string, CP_OPERATOR_STR, CP_OPERATOR_LEN) == 0
	  && !valid_symbol_name_char (string[CP_OPERATOR_LEN]));
}

/* Helper for strncmp_iw and strcmp_iw.  If mode is NORMAL, behave
   like strncmp_iw.  If mode is MATCH_PARAMS, behave like
   strcmp_iw.  */
		      ...
		      strncmp_iw_mode mode)
{
  ...
  while (1)
    {
      if (skip_spaces
	  || ((isspace (*string1) && !valid_symbol_name_char (*string2))
	      || (isspace (*string2) && !valid_symbol_name_char (*string1))))
	{
      ...
      else if (*string1 == 'o')
	{
	  if (cp_is_operator (string1, string1_start))
	      ...
	      if (string2 != end_str2)
		{
		  ...
		  if (valid_symbol_name_char (*string2))
		    return 1;
	...
	break;
      ...
      /* If we see any non-whitespace, non-symbol-name character (any
	 of "()<>*&" etc.), then skip spaces the next time around.  */
      if (!isspace (*string1) && !valid_symbol_name_char (*string1))
	skip_spaces = true;
